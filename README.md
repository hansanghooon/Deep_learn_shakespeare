# 데이터 사이언스 프로젝트: 셰익스피어 텍스트 생성 모델

본 리포지토리는 데이터 사이언스 역량 증명 및 공모전 참여를 위해 진행한 프로젝트입니다. **공개 데이터(셰익스피어 텍스트 코퍼스)**를 활용하여, 딥러닝 모델을 통한 텍스트 생성 과제를 수행했습니다.

이 프로젝트의 핵심 목표는 데이터의 특성을 파악하고(전처리), 다양한 모델(RNN, LSTM)을 실험하며, 체계적인 하이퍼파라미터 튜닝을 통해 최적의 성능을 도출하는 분석 능력을 보여주는 것입니다.

본 README 파일은 전체 분석 과정, 코드의 기능, 그리고 실험 결과를 상세히 설명하기 위해 작성되었습니다.

shakespeare (원본 프로젝트 설명)
